        ...
        contentType =
            when (contentType) {
                ...
                ContentType.CHANNELS -> {
                    YouTubeSearchParams.ContentType.CHANNEL
                }

                ContentType.PLAYLISTS -> {
                    YouTubeSearchParams.ContentType.PLAYLIST
                }
            },
        duration =
            if (contentType.supportsVideoFilters()) {
                when (duration) {
                    Duration.ANY -> null
...
internal fun WebSearchItem.toSearchResultItem(): SearchResultItem =
    when (this) {
        is SearchVideoItem -> {
            val channelThumbnailUrls =
                this.channelThumbnailUrls
                    .map(String::trim)
